...
def train():
    ...
    model = backbone(wandb.config, include_top=True, classification=True, classes=1)
    ...
    model.fit(train_set[0], train_set[1][:,0], batch_size=64,class_weight=class_weight_dict, epochs=300, validation_data=(valid_set[0], valid_set[1][:,0]), callbacks=callbacks, shuffle=True)#valid_set[1]), callbacks=callbacks
    ...
    model = load_model('model-best.h5',custom_objects=custom_objects, compile=False)
    model.summary()
...
if __name__ == '__main__':
    wandb.init(project='s3s4detect5_19', entity='luke1219')
    ...
    set_wandb_config({
        'sampling_rate': 1000 if 'audicor_10s' in wandb.config.datasets else 1000,
