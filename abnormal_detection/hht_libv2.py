...
def plot_imfs(signal, imfs, time_samples=None, fig=None):
    ...
    plt.savefig("plot_imfs.png")
    return ax



def plot_amp(signal, imfs, time_samples=None, fig=None):
    """
    plot_imfs function for the Hilbert Huang Transform is adopted from pyhht.
    Author: jaidevd https://github.com/jaidevd/pyhht/blob/dev/pyhht/visualization.py
    Plot the signal, IMFs.
    ...
    ax.grid(False)
    ax.set_ylabel('Signal')
    ax.set_title('amp of IMFs')
    #print('2n_imfs',n_imfs)
    # Plot the IMFs
    for i in range(n_imfs - 1):#n_imfs - 1):
        print(i + 2)
        ax = plt.subplot(n_imfs + 1, 1, i + 2)
        ax.plot(time_samples, imfs[i, :10000],'bo',markersize=0.5)
        
        # ax.axis([time_samples[0], time_samples[-1], -axis_extent, axis_extent])
        #ax.tick_params(which='both', left=True, bottom=False, labelleft=False,labelbottom=False)
        #ax.yaxis.tick_right()
    ...
    # Plot the residue
    ax = plt.subplot(n_imfs + 1, 1, n_imfs + 1)#n_imfs
    ax.plot(time_samples, imfs[-1, :10000], 'r',markersize=0.5)
    ax.set_ylim((0.0, 1.0))
    ax.axis('tight')
    #ax.tick_params(which='both', left=False, bottom=False, labelleft=False,
                   #labelbottom=False)
    ax.grid(False)
    #ax.set_ylabel('res.')
    plt.savefig("plot_amp.png")
    fig = plt.figure(figsize=[20,10])
    ax = fig.add_subplot(1,1,1)    
    ax.plot(time_samples, imfs[2, :10000])
    ax.set_ylim(0.0, 1.0)
...
def FAhilbert(imfs, dt):
    ...
    for i in range(n_imfs - 1):
        ...
        inst_freq =  np.diff(phase)/(2 * math.pi) * fs
...
def hht(data, time, freqsol=33, timesol=50):
    ...
    #plot_amp(data, amp.T, time_samples=time, fig=fig2)
    
    #fig3 = plt.figure(figsize=(18, 12))
    #plot_frequency(data, freq.T, time_samples=time, fig=fig3)
