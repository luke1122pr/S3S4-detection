...
def train():
    ...
    print('label',np.sum(train_set_label),np.sum(valid_set_label),np.sum(test_set_label))
    ...
    model.fit(train_set[0], train_set[1], batch_size=64, epochs=700, validation_data=(valid_set[0], valid_set[1]), callbacks=callbacks, shuffle=True)
